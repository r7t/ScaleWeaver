...
程序已拆分为 `frontend_generate.py`、`frontend_ir.py` 和
`accompaniment_generate.py`：旧前端先生成 `ScaleWeaverFrontEndIR/1`，其中
保存和声进行、Lead 旋律与独立的 Lead 节奏骨架；后端读取 IR 后生成
Bass/Inner/Counter，并继续使用原退火、统计和导出模块。原 `main.py` 一步入口
同样经过该 IR。命令和格式说明见 `FRONTEND_IR.md`。

## 同时起音的混合 CSE 软上限

风格文件现在可用 `attack_cse_percentile_limits` 分别设置二、三、四音的
百分位上限。百分位按当前 `a*CSE + b*SE + c*CCSE` 混合分数重新排序，
